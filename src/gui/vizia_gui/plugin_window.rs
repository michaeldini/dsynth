...
use vizia::prelude::WindowHandle as ViziaBaseviewWindowHandle;
use vizia::prelude::*;
...
pub fn open_editor(
    ...
    let handle = Application::new(move |cx| {
        ...
        // TODO: Wire up parameter updates once VIZIA event system is understood
        // For now, just display the layout
...
/// Build the main UI layout - Phase 2: Add envelope controls
fn build_ui(cx: &mut Context) {
    // Debug: Log UI build start
    let debug_msg = "DEBUG: build_ui called - Starting main UI layout\n";
    let _ = std::fs::OpenOptions::new()
        .create(true)
        .append(true)
        .open("/tmp/dsynth_debug.log")
    ...
    // Sized to fit the widest per-oscillator row (6 knobs) without forcing
    // label/knob overlap.
    ...
    VStack::new(cx, |cx| {
        // Title bar + live status text (feedback when dragging controls)
        HStack::new(cx, |cx| {
        ...
        // Scrollable content area (lets us keep appending sections)
        ScrollView::new(cx, |cx| {
            // Important: ensure the scroll content can grow taller than the viewport.
            // If the root stack stretches to the viewport height, VIZIA won't detect
            // overflow and vertical scrolling won't activate.
            VStack::new(cx, |cx| {
                // Row 1: Master + Envelope (keep Master top-left)
                HStack::new(cx, |cx| {
                    VStack::new(cx, |cx| {
                        Label::new(cx, "Master")
                            .font_size(16.0)
                ...
                // Row 2: Oscillators (3 aligned columns)
                VStack::new(cx, |cx| {
                    HStack::new(cx, |cx| {
                        VStack::new(cx, |cx| build_osc_section(cx, 1))
                            .width(Pixels(OSC_COL_WIDTH))
                            .height(Pixels(0.0)) // Let content define height
                            .padding(Pixels(12.0))
                            .gap(Pixels(12.0));
                ...
                // Row 3: Harmonics (3 aligned columns)
                VStack::new(cx, |cx| {
                    // Label::new(cx, "Additive Harmonics")
                    //     .font_size(16.0)
                    //     .color(Color::rgb(200, 200, 210))
                    //     .height(Pixels(12.0));
                ...
                // Row 4: Filters (3 aligned columns)
                VStack::new(cx, |cx| {
            ...
            .width(Stretch(1.0))
            .height(Units::Auto)
            .min_height(Pixels(0.0)) // Override VIZIA's default min-height: 100% on scroll-content
            .padding(Pixels(10.0))
            .gap(Pixels(ROW_GAP));
        })
...
#[allow(dead_code)]
fn build_additive_section(cx: &mut Context) {
    // Deprecated by the new row-based layout; kept for now to avoid churn if other
    // code paths still call it.
    VStack::new(cx, |cx| {
        Label::new(cx, "Additive Harmonics")
            .font_size(16.0)
            .color(Color::rgb(200, 200, 210))
...
fn build_additive_osc_section(cx: &mut Context, osc_index: usize) {
    use crate::gui::vizia_gui::widgets::param_knob;
...
fn build_effects_section(cx: &mut Context) {
    // Must fit 4 columns inside a 1200px editor without horizontal scrolling.
...
fn build_reverb_section(cx: &mut Context) {
    use crate::gui::vizia_gui::widgets::param_knob;
    use crate::plugin::param_descriptor::{
        PARAM_REVERB_DAMPING, PARAM_REVERB_DRY, PARAM_REVERB_ROOM_SIZE, PARAM_REVERB_WET,
        PARAM_REVERB_WIDTH,
    };
...
fn build_delay_section(cx: &mut Context) {
    use crate::gui::vizia_gui::widgets::param_knob;
    use crate::plugin::param_descriptor::{
        PARAM_DELAY_DRY, PARAM_DELAY_FEEDBACK, PARAM_DELAY_TIME_MS, PARAM_DELAY_WET,
    };
...
fn build_chorus_section(cx: &mut Context) {
    use crate::gui::vizia_gui::widgets::param_knob;
    use crate::plugin::param_descriptor::{
        PARAM_CHORUS_DEPTH, PARAM_CHORUS_MIX, PARAM_CHORUS_RATE,
    };
...
fn build_distortion_section(cx: &mut Context) {
    use crate::gui::vizia_gui::widgets::param_knob;
    use crate::plugin::param_descriptor::{
        PARAM_DISTORTION_DRIVE, PARAM_DISTORTION_MIX, PARAM_DISTORTION_TYPE,
    };
...
fn current_normalized(cx: &mut Context, param_id: u32) -> f32 {
...
fn default_normalized(param_id: u32) -> f32 {
    let registry = param_registry::get_registry();
    registry.get(param_id).map(|d| d.default).unwrap_or(0.0)
}

fn build_master_section(cx: &mut Context) {
    use crate::gui::vizia_gui::widgets::{param_checkbox, param_knob};
    use crate::plugin::param_descriptor::{PARAM_MASTER_GAIN, PARAM_MONOPHONIC};
...
/// Build envelope section with ADSR knobs
fn build_envelope_section(cx: &mut Context) {
    use crate::gui::vizia_gui::widgets::param_knob;

    use crate::plugin::param_descriptor::{
        PARAM_ENVELOPE_ATTACK, PARAM_ENVELOPE_DECAY, PARAM_ENVELOPE_RELEASE, PARAM_ENVELOPE_SUSTAIN,
    };
...
fn build_osc_section(cx: &mut Context, osc_index: usize) {
    use crate::gui::vizia_gui::widgets::param_knob;
    use crate::plugin::param_descriptor::*;
    ...
    VStack::new(cx, |cx| {
        ...
        HStack::new(cx, |cx| {
            ...
            // param_knob(cx, phase, "Phase", phase_v, default_normalized(phase));
            param_knob(cx, shape, "Shape", shape_v, default_normalized(shape));
        })
        .height(Units::Auto)
        .gap(Pixels(6.0));
    })
    // Two-row sections need a bit more breathing room to avoid the
    // “smooshed together” look.
    .gap(Pixels(10.0));
}

fn build_filter_section(cx: &mut Context, filter_index: usize) {
    use crate::gui::vizia_gui::widgets::param_knob;
    use crate::plugin::param_descriptor::*;
...
fn build_lfo_section(cx: &mut Context, lfo_index: usize) {
    use crate::gui::vizia_gui::widgets::param_knob;
    use crate::plugin::param_descriptor::*;
...
fn build_velocity_section(cx: &mut Context) {
    use crate::gui::vizia_gui::widgets::param_knob;
    use crate::plugin::param_descriptor::{PARAM_VELOCITY_AMP, PARAM_VELOCITY_FILTER};
