...
mod tests {
    ...
    fn test_fractional_delay_interpolation() {
        ...
        assert!(left >= 0.0 && left <= 1.0, "Interpolated value should be in range");
    }
    ...
    fn test_detune_affects_delay() {
        ...
        assert!(doubler.delay_samples > base_delay, "Positive detune should increase delay");
        
        // Negative detune = shorter delay (pitch up)
        doubler.set_detune(-10.0);
        assert!(doubler.delay_samples < base_delay, "Negative detune should decrease delay");
    }
}
