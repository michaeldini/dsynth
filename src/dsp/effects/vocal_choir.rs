    ...
    fn rebuild_voices(&mut self) {
        ...
        for i in 0..self.num_voices {
            ...
            let pan = if self.num_voices > 1 {
                -self.stereo_spread
                    + (i as f32 / (self.num_voices - 1) as f32) * 2.0 * self.stereo_spread
            } else {
                0.0 // Center for single voice
            };

            let voice = ChoirVoice::new(self.max_delay_samples, delay_ms, detune_cents, pan);
...
mod tests {
    ...
    fn test_voice_distribution() {
        ...
        assert!(
            center_voice.detune_cents.abs() < 7.0,
            "Center voice should be near 0 cents"
        );

        let outer_voice = &choir.voices[3]; // Last voice
        assert!(
            outer_voice.detune_cents.abs() > 5.0,
            "Outer voice should be detuned"
        );

        // Check stereo panning
        assert!(
            choir.voices[0].pan < -0.3,
            "First voice should be panned left"
        );
        assert!(
            choir.voices[3].pan > 0.3,
    ...
    fn test_stereo_output() {
        ...
        assert!(
            diff > 0.01,
            "Stereo spread should create L/R difference, got diff={}",
            diff
        );
        ...
        assert!(
            left2.abs() < 2.0,
            "Left output should be in reasonable range"
        );
        assert!(
            right2.abs() < 2.0,
            "Right output should be in reasonable range"
        );
    }
