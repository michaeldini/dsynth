...
mod tests {
    ...
    fn test_process_boost() {
        ...
        for i in 0..5000 {  // Run longer to allow filter settling
            let t = i as f32 / sample_rate;
            let input = (2.0 * std::f32::consts::PI * freq * t).sin() * 0.1;
            let (left, _) = eq.process(input, input);
            if i > 1000 {  // Skip initial settling time
        ...
        assert!(max_output > 0.12, "Boost should increase signal amplitude (got {})", max_output);
    }
    ...
    fn test_process_cut() {
        ...
        assert!(max_output < 0.095, "Cut should reduce signal amplitude (got {})", max_output);
    }
