...
mod tests {
    ...
    fn test_autopan_modulates_stereo() {
        ...
        let left_max = left_outputs.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
        assert!(left_max - left_min > 0.3);
        
        // Right channel should vary
        let right_min = right_outputs.iter().cloned().fold(f32::INFINITY, f32::min);
        let right_max = right_outputs.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
    ...
    fn test_autopan_depth_control() {
        ...
        let var_full = full_depth_left.iter().map(|&x| x * x).sum::<f32>() / full_depth_left.len() as f32;
