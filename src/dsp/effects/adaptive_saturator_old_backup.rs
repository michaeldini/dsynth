...
/// # Architecture
/// ```text
/// Input → Stage 1 (35% drive) → Stage 2 (25% drive) → Harmonic Synthesis (40% drive) → Auto-Gain → Output
///          ↑                      ↑                      ↑
///          └─── Transient boost + Sibilance reduction + Frequency adaptation ───┘
/// ```
///
/// **Hybrid Approach**: Combines 2-stage waveshaping (analog warmth) with direct harmonic
/// synthesis (precise spectral control). Default ratio: 60% waveshaping, 40% harmonics.
///
/// # Adaptive Processing
...
/// - Drive scaling: drive^3.0 for smooth control (subtle at low values, aggressive at high)
/// - Hybrid split: 60% waveshaping (2 stages: 35%/25%), 40% harmonic synthesis
/// - Harmonic intensity: 0.5 (50%) for balanced spectral enhancement
...
pub struct AdaptiveSaturator {
    ...
    // DC blocking filters (per stage, supports up to 3 stages)
    dc_block_x1: [f32; 3],
    dc_block_y1: [f32; 3],
    dc_block_coeff: f32,

    // Harmonic synthesis controls
    harmonic_intensity: f32, // 0.0-1.0, multiplier for all harmonic amplitudes
    waveshaping_ratio: f32,  // 0.0-1.0, where 1.0=100% waveshaping, 0.0=100% harmonics
}
...
impl AdaptiveSaturator {
    ...
    pub fn new(sample_rate: f32) -> Self {
        ...
        Self {
            ...
            dc_block_y1: [0.0; 3],
            dc_block_coeff,
            harmonic_intensity: 0.5, // Default: balanced harmonic content
            waveshaping_ratio: 0.6,  // Default: 60% waveshaping, 40% harmonics
        }
    }

    /// Set harmonic intensity (0.0-1.0)
    ///
    /// Controls the amplitude of all generated harmonics.
    /// 0.0 = no harmonics, 0.5 = balanced, 1.0 = maximum harmonic content
    pub fn set_harmonic_intensity(&mut self, intensity: f32) {
        self.harmonic_intensity = intensity.clamp(0.0, 1.0);
    }

    /// Set waveshaping/harmonics ratio (0.0-1.0)
    ///
    /// Controls the balance between waveshaping stages and direct harmonic synthesis.
    /// 1.0 = 100% waveshaping (pure analog emulation)
    /// 0.6 = 60% waveshaping, 40% harmonics (default, balanced)
    /// 0.0 = 100% harmonics (pure spectral synthesis)
    pub fn set_waveshaping_ratio(&mut self, ratio: f32) {
        self.waveshaping_ratio = ratio.clamp(0.0, 1.0);
    }
    ...
    pub fn process(
        ...
        // Hybrid architecture: Split drive between waveshaping and harmonic synthesis
        // Waveshaping provides analog warmth, harmonics provide precise spectral control
        let waveshaping_drive = adaptive_drive * self.waveshaping_ratio;
        let harmonics_drive = adaptive_drive * (1.0 - self.waveshaping_ratio);

        // Calculate 2-stage distribution (35%/25% for analog character)
        // Normalized to maintain consistent energy when ratio changes
        let stage1_drive = waveshaping_drive * 0.35 / 0.60; // 35% of total drive
        let stage2_drive = waveshaping_drive * 0.25 / 0.60; // 25% of total drive

        // Track input RMS for auto-gain compensation
        let mono_input = (left + right) * 0.5;
        self.update_rms_input(mono_input);

        // Process left channel through 2-stage waveshaping cascade
        ...
        // Add harmonic synthesis (sibilance-aware)
        left_out += self.synthesize_harmonics(
            left,
            harmonics_drive,
            character,
            analysis.sibilance_strength,
        );

        // Process right channel through 2-stage waveshaping cascade
        ...
        // Add harmonic synthesis (sibilance-aware)
        right_out += self.synthesize_harmonics(
            right,
            harmonics_drive,
            character,
            analysis.sibilance_strength,
    ...
    /// Synthesize harmonics directly with character-specific profiles
    ///
    /// This method generates harmonics using pure sine waves rather than waveshaping,
    /// allowing precise spectral control. Sibilance-aware: reduces high-frequency
    /// harmonics (5th, 7th) during sibilant content to preserve vocal clarity.
    ///
    /// # Arguments
    /// * `input` - Input sample
    /// * `drive` - Drive amount (0.0-1.0)
    /// * `character` - Saturation character (determines harmonic profile)
    /// * `sibilance_strength` - Sibilance level (0.0-1.0) for high-frequency reduction
    #[inline]
    fn synthesize_harmonics(
        &self,
        input: f32,
        drive: f32,
        character: SaturationCharacter,
        sibilance_strength: f32,
    ) -> f32 {
        let abs_x = input.abs();
        let phase = input * std::f32::consts::PI;

        // Level-dependent harmonic generation (stronger harmonics on louder signals)
        let level_factor = abs_x;

        // Generate character-specific harmonic series
        // Sibilance-aware: reduces high-frequency harmonics (5th, 7th) during sibilant content
        let harmonics = match character {
            SaturationCharacter::Warm => {
                // Tube-style: Strong 2nd harmonic (even), gentle 3rd/4th
                // 2nd harmonic unaffected by sibilance (low frequency)
                let h2 = (phase * 2.0).sin() * 0.20 * drive * level_factor;
                let h3 = (phase * 3.0).sin()
                    * 0.10
                    * drive
                    * level_factor
                    * (1.0 - sibilance_strength * 0.6);
                let h4 = (phase * 4.0).sin()
                    * 0.05
                    * drive
                    * level_factor
                    * (1.0 - sibilance_strength * 0.6);
            ...
            SaturationCharacter::Smooth => {
                // Tape-style: Balanced odd harmonics (3rd, 5th, 7th)
                // 3rd harmonic less affected, 5th/7th heavily reduced on sibilance
                let h3 = (phase * 3.0).sin() * 0.15 * drive * level_factor;
                let h5 = (phase * 5.0).sin()
                    * 0.08
                    * drive
                    * level_factor
                    * (1.0 - sibilance_strength * 0.8);
                let h7 = (phase * 7.0).sin()
                    * 0.04
                    * drive
                    * level_factor
                    * (1.0 - sibilance_strength * 0.8);
            ...
            SaturationCharacter::Punchy => {
                // Console-style: Aggressive 3rd/5th, strong presence (7th)
                // High frequencies reduced on sibilance to prevent harshness
                let h3 = (phase * 3.0).sin() * 0.25 * drive * level_factor;
                let h5 = (phase * 5.0).sin()
                    * 0.15
                    * drive
                    * level_factor
        ...
        // Apply global harmonic intensity control
        harmonics * self.harmonic_intensity
    }
...
mod tests {
    ...
    fn test_sibilance_aware_saturation() {
        ...
        // Both outputs should be valid (auto-gain compensation keeps levels similar)
        // The actual difference is in harmonic content, not raw amplitude
    ...
    fn test_voiced_unvoiced_adaptation() {
        ...
        // Both outputs should be valid (auto-gain compensation keeps levels similar)
        // The actual difference is in harmonic content and drive level, not raw amplitude
        // Just verify no crash and finite output
    }
    ...
    fn test_zcr_frequency_adaptation() {
        ...
        // All outputs should be valid (auto-gain compensation keeps levels similar)
        // The actual difference is in drive level and harmonic content, not raw amplitude
        // Just verify no crash and finite output
    }

    #[test]
    fn test_harmonic_intensity_scaling() {
        let mut saturator = AdaptiveSaturator::new(44100.0);
        let analysis = create_test_analysis();
        let input = 0.5;

        // Test with zero intensity (no harmonics)
        saturator.set_harmonic_intensity(0.0);
        for _ in 0..1000 {
            saturator.process(input, input, 0.5, 1.0, SaturationCharacter::Warm, &analysis);
        }
        let (out_zero, _) =
        ...
        // Test with full intensity (maximum harmonics)
        saturator = AdaptiveSaturator::new(44100.0);
        saturator.set_harmonic_intensity(1.0);
        for _ in 0..1000 {
            saturator.process(input, input, 0.5, 1.0, SaturationCharacter::Warm, &analysis);
        }
        let (out_full, _) =
        ...
        assert!(out_full.is_finite());

        // Full intensity should produce more pronounced effect (but auto-gain may compensate)
        // Just verify no crash and valid output
    }

    #[test]
    fn test_waveshaping_ratio_extremes() {
        let mut saturator = AdaptiveSaturator::new(44100.0);
        let analysis = create_test_analysis();
        let input = 0.5;

        // Test pure waveshaping (ratio = 1.0)
        saturator.set_waveshaping_ratio(1.0);
        for _ in 0..1000 {
            saturator.process(input, input, 0.5, 1.0, SaturationCharacter::Warm, &analysis);
        }
        let (out_pure_wave, _) =
        ...
        // Test pure harmonics (ratio = 0.0)
        saturator = AdaptiveSaturator::new(44100.0);
        saturator.set_waveshaping_ratio(0.0);
        for _ in 0..1000 {
            saturator.process(input, input, 0.5, 1.0, SaturationCharacter::Warm, &analysis);
        }
        let (out_pure_harm, _) =
        ...
        // Test balanced (ratio = 0.6, default)
        saturator = AdaptiveSaturator::new(44100.0);
        saturator.set_waveshaping_ratio(0.6);
        for _ in 0..1000 {
            saturator.process(input, input, 0.5, 1.0, SaturationCharacter::Warm, &analysis);
        }
        let (out_balanced, _) =
        ...
        assert!(out_pure_wave.is_finite());
        assert!(out_pure_harm.is_finite());
        assert!(out_balanced.is_finite());

        // All should produce valid output in reasonable range
        assert!(out_pure_wave.abs() < 2.0);
        assert!(out_pure_harm.abs() < 2.0);
        assert!(out_balanced.abs() < 2.0);
    }

    #[test]
    fn test_sibilance_reduces_high_harmonics() {
        let mut saturator = AdaptiveSaturator::new(44100.0);
        let input = 0.5;

        // Test with low sibilance (full harmonics)
        let mut analysis_low = create_test_analysis();
        analysis_low.sibilance_strength = 0.0;
        ...
        // Test with high sibilance (reduced 5th/7th harmonics)
        saturator = AdaptiveSaturator::new(44100.0);
        let mut analysis_high = create_test_analysis();
        analysis_high.sibilance_strength = 1.0;
        ...
        // High sibilance should be closer to input (less high-frequency harmonics)
        let low_sib_diff = (out_low_sib - input).abs();
        let high_sib_diff = (out_high_sib - input).abs();

        // With sibilance, output should be gentler (closer to input)
        assert!(
            high_sib_diff < low_sib_diff,
            "High sibilance should reduce harmonics. Low: {}, High: {}",
            low_sib_diff,
            high_sib_diff
        );
    }
    ...
    #[test]
    fn test_warm_2nd_harmonic_preserved_on_sibilance() {
        let saturator = AdaptiveSaturator::new(44100.0);

        // Test Warm character with high sibilance
        // 2nd harmonic should be unaffected (low frequency)
        // Only 3rd/4th harmonics should be reduced

        let input = 0.3;
        let drive = 0.7;

        // Low sibilance
        let harmonics_low = saturator.synthesize_harmonics(
            input,
            drive,
            SaturationCharacter::Warm,
            0.0, // No sibilance
        ...
        // High sibilance
        let harmonics_high = saturator.synthesize_harmonics(
            input,
            drive,
            SaturationCharacter::Warm,
            1.0, // Maximum sibilance
        );

        // Both should be finite
        assert!(harmonics_low.is_finite());
        assert!(harmonics_high.is_finite());

        // High sibilance should produce less harmonics (3rd/4th reduced)
        assert!(
            harmonics_high.abs() < harmonics_low.abs(),
            "Sibilance should reduce 3rd/4th harmonics. Low: {}, High: {}",
            harmonics_low,
            harmonics_high
        );
    }
    ...
    #[test]
    fn test_hybrid_all_characters_finite() {
        let mut saturator = AdaptiveSaturator::new(44100.0);
        let analysis = create_test_analysis();

        // Test all combinations of character, drive, and intensity
        let characters = [
            SaturationCharacter::Warm,
            SaturationCharacter::Smooth,
            SaturationCharacter::Punchy,
        ...
        for &character in &characters {
            for &drive in &[0.0, 0.5, 1.0] {
                for &intensity in &[0.0, 0.5, 1.0] {
                    saturator.set_harmonic_intensity(intensity);

                    // Warm up RMS
                    for _ in 0..100 {
                        saturator.process(0.5, 0.5, drive, 1.0, character, &analysis);
                    }
                    ...
                    assert!(
                        out_l.is_finite(),
                        "Character {:?}, drive {}, intensity {} produced NaN/Inf",
                        character,
                        drive,
                        intensity
                    );
    ...
    #[test]
    fn test_default_60_40_split() {
        let saturator = AdaptiveSaturator::new(44100.0);

        // Default should be 60% waveshaping, 40% harmonics
        assert_relative_eq!(saturator.waveshaping_ratio, 0.6, epsilon = 0.01);
        assert_relative_eq!(saturator.harmonic_intensity, 0.5, epsilon = 0.01);
    }

    #[test]
    fn test_setter_methods_clamp_values() {
        let mut saturator = AdaptiveSaturator::new(44100.0);

        // Test clamping of harmonic intensity
        saturator.set_harmonic_intensity(-0.5);
        assert_eq!(saturator.harmonic_intensity, 0.0);

        saturator.set_harmonic_intensity(1.5);
        assert_eq!(saturator.harmonic_intensity, 1.0);

        // Test clamping of waveshaping ratio
        saturator.set_waveshaping_ratio(-0.5);
        assert_eq!(saturator.waveshaping_ratio, 0.0);

        saturator.set_waveshaping_ratio(1.5);
        assert_eq!(saturator.waveshaping_ratio, 1.0);
    }
}
