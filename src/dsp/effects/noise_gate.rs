...
impl NoiseGate {
    ...
    pub fn process(&mut self, left: f32, right: f32) -> (f32, f32) {
        ...
        self.rms_envelope_left =
            self.rms_envelope_left * self.rms_coeff + left_squared * (1.0 - self.rms_coeff);
        self.rms_envelope_right =
            self.rms_envelope_right * self.rms_coeff + right_squared * (1.0 - self.rms_coeff);
...
mod tests {
    ...
    fn test_gate_blocks_quiet_signal() {
        ...
        assert!(
            output_sum < input_sum * 0.5,
            "Gate should reduce quiet signal"
        );
    }
