...
impl IntelligentDeEsser {
    ...
    pub fn process(
        ...
        self.process_with_hf_delta(left, right, threshold, amount, analysis)
            .0
    }

    /// Process stereo sample and also return the **HF reduction delta**.
    ///
    /// Returns:
    ...
    pub fn process_with_hf_delta(
        ...
        let det_attack_coeff = (-1.0 / (det_attack_s * self.sample_rate)).exp();
        let det_release_coeff = (-1.0 / (det_release_s * self.sample_rate)).exp();
        ...
        let gain_attack_coeff = (-1.0 / (gain_attack_s * self.sample_rate)).exp();
        let gain_release_coeff = (-1.0 / (gain_release_s * self.sample_rate)).exp();
...
mod tests {
    ...
    fn test_bypass_amount_zero_is_bit_perfect() {
        let mut de_esser = IntelligentDeEsser::new(44100.0);
        let analysis = analysis_with_strength(1.0);

        let (out_l, out_r) = de_esser.process(0.1234, -0.4321, 0.8, 0.0, &analysis);
    ...
    fn test_no_reduction_when_threshold_is_max() {
        ...
        let mut de_esser = IntelligentDeEsser::new(sample_rate);
        let analysis = analysis_with_strength(1.0);
        ...
        for i in 0..4096 {
            ...
            let (out_l, _) = de_esser.process(input, input, threshold, amount, &analysis);
    ...
    fn test_reduces_high_frequency_energy() {
        ...
        for i in 0..2048 {
            ...
            let _ = de_esser.process(input, input, threshold, amount, &analysis);
        }
        ...
        let in_rms = (in_sum / 4096.0).sqrt();
        let out_rms = (out_sum / 4096.0).sqrt();
