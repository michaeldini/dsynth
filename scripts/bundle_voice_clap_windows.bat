@echo off
REM Bundle script for creating DSynth Voice CLAP plugin on Windows

setlocal enabledelayedexpansion

set PLUGIN_NAME=DSynthVoice
if not "%TARGET%"=="" (
...
echo Building DSynth Voice CLAP plugin...

REM Build the CLAP plugin
echo Building release binary...
if not "%TARGET%"=="" (
    cargo build --release --lib --no-default-features --features voice-clap --target %TARGET%
) else (
    cargo build --release --lib --no-default-features --features voice-clap
)
